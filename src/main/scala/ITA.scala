...
// ITA BlackBox - uses packed UInt signals to avoid flattening
class ITAMMIOBlackBox(params: ITAParams) extends BlackBox with HasBlackBoxResource {
  val io = IO(new Bundle {
    val clk_i = Input(Clock())
    val rst_ni = Input(Reset())

    // Packed control signal (as UInt to avoid flattening)
    val ctrl_i = Input(UInt((1 + 2 + 2 + params.N_REQUANT_CONSTS * params.EMS * 2 + 
                             params.N_REQUANT_CONSTS * params.WI + 16 * 2 + 
                             params.EMS * 2 + params.WI + 32 * 4).W))

    val inp_valid_i = Input(Bool())
    val inp_ready_o = Output(Bool())
    val inp_weight_valid_i = Input(Bool())
    val inp_weight_ready_o = Output(Bool())
    val inp_bias_valid_i = Input(Bool())
    val inp_bias_ready_o = Output(Bool())
    val valid_o = Output(Bool())
    val ready_i = Input(Bool())
    val busy_o = Output(Bool())

    // Packed vectors as UInt to avoid flattening
    val inp_i = Input(UInt((params.M * params.WI).W))
    val inp_weight_i = Input(UInt((params.N * params.M / params.N_WRITE_EN * params.WI).W))
    val inp_bias_i = Input(UInt((params.N * (params.WO - 2)).W))
    val oup_o = Output(UInt((params.N * params.WI).W))
  })
  ...
  // Main modules
  addResource("/vsrc/ITAMMIOBlackBox.sv")
  addResource("/vsrc/ita.sv")
  
  // Sub-modules required by ita.sv
  addResource("/vsrc/ita_controller.sv")
  addResource("/vsrc/ita_input_sampler.sv")
  addResource("/vsrc/ita_inp1_mux.sv")
  addResource("/vsrc/ita_inp2_mux.sv")
  addResource("/vsrc/ita_sumdotp.sv")
  addResource("/vsrc/ita_accumulator.sv")
  addResource("/vsrc/ita_softmax_top.sv")
  addResource("/vsrc/ita_requantization_controller.sv")
  addResource("/vsrc/ita_requantizer.sv")
  addResource("/vsrc/ita_activation.sv")
  addResource("/vsrc/ita_fifo_controller.sv")
  addResource("/vsrc/ita_output_controller.sv")
  addResource("/vsrc/ita_weight_controller.sv")
  addResource("/vsrc/ita_register_file_1w_multi_port_read_we.sv")
  
  // Additional modules that may be required
  addResource("/vsrc/ita_dotp.sv")
  addResource("/vsrc/ita_gelu.sv")
  addResource("/vsrc/ita_relu.sv")
  addResource("/vsrc/ita_softmax.sv")
  addResource("/vsrc/ita_max_finder.sv")
  addResource("/vsrc/ita_serdiv.sv")
  addResource("/vsrc/ita_register_file_1w_1r_double_width_write.sv")
  addResource("/vsrc/ita_register_file_1w_multi_port_read.sv")
...
// Chisel wrapper that packs Vecs/Bundles into UInts for the BlackBox
class ITAWrapper(params: ITAParams) extends Module {
  val io = IO(new Bundle {
    val clk_i = Input(Clock())
    val rst_ni = Input(Reset())
    ...
    val ready_i = Input(Bool())
    val busy_o = Output(Bool())
    val inp_i = Input(Vec(params.M, SInt(params.WI.W)))
    val inp_weight_i = Input(Vec(params.N * params.M / params.N_WRITE_EN, SInt(params.WI.W)))
    val inp_bias_i = Input(Vec(params.N, SInt((params.WO - 2).W)))
    val oup_o = Output(Vec(params.N, SInt(params.WI.W)))
  })

  val blackbox = Module(new ITAMMIOBlackBox(params))
  
  // Pack control bundle into UInt to match SystemVerilog struct order:
  // start, layer, activation, eps_mult[0..7], right_shift[0..7], add[0..7], 
  ...
  val ctrl_packed = Cat(
    ...
    io.ctrl_i.tile_e,
    io.ctrl_i.tile_s,
    io.ctrl_i.activation_requant_add.asUInt,
    io.ctrl_i.activation_requant_shift,
    io.ctrl_i.activation_requant_mult,
    io.ctrl_i.gelu_c.asUInt,
    io.ctrl_i.gelu_b.asUInt,
    // add array (packed in reverse order for Cat)
    io.ctrl_i.add(7).asUInt, io.ctrl_i.add(6).asUInt, io.ctrl_i.add(5).asUInt, io.ctrl_i.add(4).asUInt,
    io.ctrl_i.add(3).asUInt, io.ctrl_i.add(2).asUInt, io.ctrl_i.add(1).asUInt, io.ctrl_i.add(0).asUInt,
    // right_shift array
    io.ctrl_i.right_shift(7), io.ctrl_i.right_shift(6), io.ctrl_i.right_shift(5), io.ctrl_i.right_shift(4),
    io.ctrl_i.right_shift(3), io.ctrl_i.right_shift(2), io.ctrl_i.right_shift(1), io.ctrl_i.right_shift(0),
    // eps_mult array
    io.ctrl_i.eps_mult(7), io.ctrl_i.eps_mult(6), io.ctrl_i.eps_mult(5), io.ctrl_i.eps_mult(4),
    io.ctrl_i.eps_mult(3), io.ctrl_i.eps_mult(2), io.ctrl_i.eps_mult(1), io.ctrl_i.eps_mult(0),
    io.ctrl_i.activation,
    io.ctrl_i.layer,
    io.ctrl_i.start
  )
  
  // Pack input vectors into UInts (Vec.asUInt packs elements in order)
  val inp_packed = io.inp_i.asUInt
  val weight_packed = io.inp_weight_i.asUInt
  val bias_packed = io.inp_bias_i.asUInt
  
  // Connect to blackbox
  blackbox.io.clk_i := io.clk_i
  blackbox.io.rst_ni := io.rst_ni
  blackbox.io.ctrl_i := ctrl_packed
  blackbox.io.inp_valid_i := io.inp_valid_i
  io.inp_ready_o := blackbox.io.inp_ready_o
  blackbox.io.inp_weight_valid_i := io.inp_weight_valid_i
  io.inp_weight_ready_o := blackbox.io.inp_weight_ready_o
  blackbox.io.inp_bias_valid_i := io.inp_bias_valid_i
  io.inp_bias_ready_o := blackbox.io.inp_bias_ready_o
  io.valid_o := blackbox.io.valid_o
  blackbox.io.ready_i := io.ready_i
  io.busy_o := blackbox.io.busy_o
  blackbox.io.inp_i := inp_packed
  blackbox.io.inp_weight_i := weight_packed
  blackbox.io.inp_bias_i := bias_packed
  
  // Unpack output
  val oup_unpacked = blackbox.io.oup_o.asTypeOf(Vec(params.N, SInt(params.WI.W)))
  io.oup_o := oup_unpacked
}
...
class ITATL(params: ITAParams, beatBytes: Int)(implicit p: Parameters) extends ClockSinkDomain(ClockSinkParameters())(p) {
  ...
  class ITAImpl extends Impl with HasITATopIO {
    ...
    withClockAndReset(clock, reset) {
      ...
      // Instantiate the ITA wrapper (which handles packing)
      val impl = Module(new ITAWrapper(params))
      ...
      // Block write data registers (written by software) - use UInt for register mapping
      val inputWriteDataUInt = RegInit(0.U(params.WI.W))
      val weightWriteDataUInt = RegInit(0.U(params.WI.W))
      val biasWriteDataUInt = RegInit(0.U((params.WO - 2).W))
      ...
      impl.io.inp_valid_i := allInputsValid
      impl.io.inp_weight_valid_i := allInputsValid
      impl.io.inp_bias_valid_i := allInputsValid

      // Connect data vectors directly
      impl.io.inp_i := inputBuffer
      impl.io.inp_weight_i := weightBuffer
      impl.io.inp_bias_i := biasBuffer

      // Output stream: always ready to capture new output
      impl.io.ready_i := true.B

      // Handle input processing completion
      when(impl.io.inp_ready_o && impl.io.inp_valid_i) {
        inputBlockValid := false.B
        weightBlockValid := false.B
        biasBlockValid := false.B
      }
      ...
      when(impl.io.valid_o && impl.io.ready_i) {
        outputBuffer := impl.io.oup_o
        outputBlockValid := true.B
      }

      // Block write operations
      when(inputWriteEn) {
        inputBuffer(inputBlockIdx) := inputWriteDataUInt.asSInt
      ...
      when(weightWriteEn) {
        weightBuffer(weightBlockIdx) := weightWriteDataUInt.asSInt
      ...
      when(biasWriteEn) {
        biasBuffer(biasBlockIdx) := biasWriteDataUInt.asSInt
      ...
      status := Cat(
        allInputsValid && impl.io.inp_ready_o,  // Ready to process when all inputs loaded and ITA ready
      ...
      node.regmap(
        ...
        0x50 -> Seq(RegField.w(params.WI, (valid: Bool, data: UInt) => {
          when(valid) { ctrl.add(0) := data.asSInt }
          true.B
        })),
        0x54 -> Seq(RegField.w(params.WI, (valid: Bool, data: UInt) => {
          when(valid) { ctrl.add(1) := data.asSInt }
          true.B
        })),
        0x58 -> Seq(RegField.w(params.WI, (valid: Bool, data: UInt) => {
          when(valid) { ctrl.add(2) := data.asSInt }
          true.B
        })),
        0x5C -> Seq(RegField.w(params.WI, (valid: Bool, data: UInt) => {
          when(valid) { ctrl.add(3) := data.asSInt }
          true.B
        })),
        0x60 -> Seq(RegField.w(params.WI, (valid: Bool, data: UInt) => {
          when(valid) { ctrl.add(4) := data.asSInt }
          true.B
        })),
        0x64 -> Seq(RegField.w(params.WI, (valid: Bool, data: UInt) => {
          when(valid) { ctrl.add(5) := data.asSInt }
          true.B
        })),
        0x68 -> Seq(RegField.w(params.WI, (valid: Bool, data: UInt) => {
          when(valid) { ctrl.add(6) := data.asSInt }
          true.B
        })),
        0x6C -> Seq(RegField.w(params.WI, (valid: Bool, data: UInt) => {
          when(valid) { ctrl.add(7) := data.asSInt }
          true.B
        })),

        // GELU constants (16-bit signed each)
        0x70 -> Seq(RegField.w(16, (valid: Bool, data: UInt) => {
          when(valid) { ctrl.gelu_b := data.asSInt }
          true.B
        })),
        0x74 -> Seq(RegField.w(16, (valid: Bool, data: UInt) => {
          when(valid) { ctrl.gelu_c := data.asSInt }
          true.B
        })),
        ...
        0x80 -> Seq(RegField.w(params.WI, (valid: Bool, data: UInt) => {
          when(valid) { ctrl.activation_requant_add := data.asSInt }
          true.B
        })),
        ...
        // Block transfer control
        0x94 -> Seq(RegField.w(params.WI, inputWriteDataUInt)), // Input data write register
        0x98 -> Seq(RegField.w(1, inputWriteEn)), // Trigger input data write to buffer
        0x9C -> Seq(RegField.w(params.WI, weightWriteDataUInt)), // Weight data write register
        0xA0 -> Seq(RegField.w(1, weightWriteEn)), // Trigger weight data write to buffer
        0xA4 -> Seq(RegField.w(params.WO - 2, biasWriteDataUInt)), // Bias data write register
